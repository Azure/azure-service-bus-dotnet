...
namespace Microsoft.Azure.ServiceBus
{
    ...
    public class Message
    {
        ...
        private byte[] body;
        private ArraySegment<byte>? backingData;
        ...
        /// <summary>
        /// Creates a new Message from the specified buffer backing storage.
        /// </summary>
        /// <param name="buffer">The buffer containing the message payload.</param>
        /// <param name="offset">The zero-based index of the first byte in the message.</param>
        /// <param name="count">The number of bytes in the message.</param>
        public Message(byte[] buffer, int offset, int count)
            : this(null)
        {
            this.backingData = new ArraySegment<byte>(buffer, offset, count);
        }
        ...
        /// </code>
        /// Note: In the event that a buffer is used as backing storage for this Message, accessing Body
        /// will cause a memory allocation and data copy from the backing storage, fixing the Body. Writing
        /// to the Body when a backing store is used will discard the backing storage completely.
        /// </remarks>
        public byte[] Body
        {
            get
            {
                if (this.backingData.HasValue)
                {
                    this.body = new byte[this.backingData.Value.Count];
                    Array.Copy(this.backingData.Value.Array, this.backingData.Value.Offset, this.body, 0, this.backingData.Value.Count);
        ...
        /// <summary>
        /// Gets the data behind the message body.
        /// </summary>
        public ArraySegment<byte> BackingData
        {
            get
            {
                if (this.backingData.HasValue)
                    return this.backingData.Value;
                if (this.body != null)
                    return new ArraySegment<byte>(this.body, 0, this.body.Length);
                return EmptyData;
            }
        }
        ...
        /// </summary>
        public long Size => this.backingData.HasValue ? this.backingData.Value.Count : this.body.Length;
        ...
        public Message Clone()
        {
            ...
            if (this.backingData.HasValue)
            {
                clone.body = new byte[this.backingData.Value.Count];
                Array.Copy(this.backingData.Value.Array, this.backingData.Value.Offset, clone.body, 0, this.backingData.Value.Count);
